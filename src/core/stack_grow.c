...
void coco_stack_overflow_handler(
    ...
    // This requires access to the scheduler's current coroutine
    // Simplified implementation

    uintptr_t fault_addr = (uintptr_t)info->si_addr;

    // Check if this is a stack overflow
    // (fault address is below current stack limit)

    // For now, just terminate
    // Full implementation would:
    // 1. Identify which coroutine overflowed
    // 2. Grow its stack
    // 3. Resume execution

    _exit(128 + sig);  // Standard exit for signal termination
}

/**
 * Setup sigaltstack for overflow handling.
 */
int coco_setup_sigaltstack(size_t altstack_size) {
    if (altstack_size == 0) {
        altstack_size = SIGSTKSZ * 2;
    }
