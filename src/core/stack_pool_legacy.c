...
/* Size class 大小表 */
static const size_t size_class_table[STACK_POOL_LEGACY_NUM_CLASSES] = {
    STACK_SIZE_8K,
    STACK_SIZE_16K,
    STACK_SIZE_32K,
    STACK_SIZE_64K,
...
int stack_pool_legacy_get_class_index(size_t size) {
    for (int i = 0; i < STACK_POOL_LEGACY_NUM_CLASSES; i++) {
...
size_t stack_pool_legacy_get_class_size(int class_idx) {
    if (class_idx < 0 || class_idx >= STACK_POOL_LEGACY_NUM_CLASSES) {
        return 0;
    }
...
stack_pool_legacy_t *stack_pool_legacy_create(void) {
    stack_pool_legacy_t *pool = calloc(1, sizeof(stack_pool_legacy_t));
    if (!pool) {
        return NULL;
    }
    ...
    for (int i = 0; i < STACK_POOL_LEGACY_NUM_CLASSES; i++) {
        ...
        pool->limits[i] = STACK_POOL_LEGACY_CLASS_LIMIT;
...
void stack_pool_legacy_destroy(stack_pool_legacy_t *pool) {
    ...
    /* 释放所有空闲栈 */
    for (int i = 0; i < STACK_POOL_LEGACY_NUM_CLASSES; i++) {
        stack_node_legacy_t *node = pool->freelists[i];
        while (node) {
            stack_node_legacy_t *next = node->next;
...
void *stack_pool_legacy_alloc(stack_pool_legacy_t *pool, size_t size) {
    ...
    int class_idx = stack_pool_legacy_get_class_index(size);
    ...
    /* 尝试从空闲链表获取 */
    stack_node_legacy_t *node = pool->freelists[class_idx];
    if (node) {
...
void stack_pool_legacy_free(stack_pool_legacy_t *pool, void *stack_top, size_t size) {
    ...
    stack_node_legacy_t *node = (stack_node_legacy_t*)((uintptr_t)stack_base + page_size);
